...
class RegisterController extends Controller
{
        ...
        if ($_POST) {
            ...
            if ($validator->fails()) {
                ...
                try {
                    ...
                    if ($request->referral) {
                        $this->user->createHistoryPoint($user->id, 'referral');
                    }
    ...
    public function checkUserRegistrationUsername(Request $request)
    {
        $username = User::where(['username' => $request->username])->exists();
        if ($username) {
            $data['status'] = true;
            $data['fail']   = __('The username has already been taken!');
        } else {
            $data['status']  = false;
    ...
    public function checkUserRegistrationReferrral(Request $request)
    {
        $referral = User::where(['username' => $request->referral])->exists();
        if ($referral) {
            $data['status'] = true;
            $data['fail']   = __('Referral Is valid');
        } else {
            $data['status']  = false;
            $data['success'] = "Referral Not Valid!";
        }
        return json_encode($data);
    }
