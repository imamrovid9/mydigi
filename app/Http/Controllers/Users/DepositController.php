...
class DepositController extends Controller
{
    ...
    public function getDepositFeesLimit(Request $request)
    {
        ...
        $user_id = auth()->user()->id;
        $cekbalance = Wallet::where(['currency_id' => 5, 'user_id' => $user_id])->first(['balance']);
        $totalbalance = $cekbalance->balance;
        if ($totalbalance <= 0) {
            @$feesDetails->min_limit = 1000000;
        }
        if (@$feesDetails->max_limit == null) {
            if ((@$amount < @$feesDetails->min_limit)) {
        ...
        if (isset($request['m_operation_id']) && isset($request['m_sign'])) {
            ...
            if ($request['m_sign'] == $sign_hash && $request['m_status'] == 'success') {
                ...
                try {
                    ...
                    \DB::rollBack();
                    session()->forget([
                        'coinpaymentAmount', 'wallet_currency_id', 'method', 'payment_method_id', 'amount', 'payeer_merchant_id', 'payeer_secret_key',
    ...
    public function bankPaymentConfirm(Request $request)
    {
        ...
        try {
            ...
            if ($request->firstdeposit == "yes") {
                $deposit->depositfirst      = "yes";
            }
