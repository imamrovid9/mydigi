...
class CustomerController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        $data['dailycheck'] = User::find(Auth::user()->id);

        $data['cektotalamount'] = Funcashback::where('user_id', Auth::user()->id)->sum('amount');
        // dd($data['dailycheck']);
    ...
    public function dailycheckin(Request $request)
    {
        ...
        if ($request->ngecek == "fdhjgrgwrhKNokMPlpdWdCfCddfCf") {
            // return $request;
            $user = User::where(['id' => Auth::user()->id])->first();
            $user->dailycheck = date('Y-m-d');
            // dd($date);
            $hasilpoint = (int)$user->xpoint;
            $user->xpoint = $hasilpoint + 1;
            // dd($user->xpoint);
            $user->save();
            $this->asd   = new User();
            $this->asd->dailyHistoryPoint($user->id, 'harian', 1);
            $this->helper->one_time_message('success', __('Success absen daily checkin!'));
            return back();
        }
    }
    ...
    protected function insertUserIdentityInfoToFilesTable($identity_file)
    {
        if (!empty($identity_file)) {
            ...
            if ($request->hasFile('identity_file')) {
                ...
                if (
                    $file_extn == 'docx' || $file_extn == 'rtf' || $file_extn == 'doc' || $file_extn == 'pdf' || $file_extn == 'png' || $file_extn == 'jpg' || $file_extn == 'jpeg' || $file_extn == 'csv'
